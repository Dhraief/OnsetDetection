...
class convNet(nn.Module):
    ...
    def train_data_builder(self, feats, answer, major_note_index, samplerate, soundlen=15, minibatch=1, split=0.2):
        ...
        Variables:
            ...
            maxspace: maximum space between major note indexs
             : index of major_note_index or feats
        ...
        if (major_note_index.shape[0] - soundlen >0):
  
            idx = np.random.permutation(major_note_index.shape[0] - soundlen) + soundlen // 2
        else:
            idx= np.array([i for i in range(major_note_index.shape[0]-1)])
        X, y = [], []
        cnt = 0


        #print(" range i ",int((idx.shape[0]-1) * split))
        for i in range(int((idx.shape[0]-1) * split)):
            #print("i",i," all",int((idx.shape[0]-1) * split),"size", idx.shape[0])
            ...
            if dist < maxspace * samplerate / 512 and dist > minspace * samplerate / 512:    
                for j in range(-1, dist + 2):
                    startIdx=major_note_index[idx[i]] - soundlen // 2 + j
                    endIdx=major_note_index[idx[i]] + soundlen // 2 + j + 1
                    if (startIdx> endIdx)or (startIdx<0):
                            print("dkhal")
                            break
                    x_to_append=feats[:, :,  startIdx: endIdx]
                    if debug:
                        print("start",startIdx,"end",endIdx)
                    if (x_to_append.shape!= (3,80,15)):
                        print("wrong appended shape",x_to_append.shape)
                    ...
                    if cnt % minibatch == 0:
                        n_X=np.array(X)
                        #print("shapes of the generated",n_X.shape)
    ...
    def train(self, songs, minibatch, epoch, device, soundlen=15, val_song=None, save_place='./models/model.pth', log='./log/log.txt', don_ka=0):
        ...
        print("Initialized the Early Stopping")
        loopmustbreak=False
        for song in songs:
            #print(song.timestamp)
            timing = song.timestamp[:, 0]
            sound  = song.timestamp[:, 1]
            #timing = [x[0] for x in song.timestamp]
            #sound = [x[1] for x in song.timestamp]
        ...
        for i in range(epoch):
            ...
            print("[",i,"/",epoch,"] train loss:",train_loss)
         #   print("loss is ", loss)
